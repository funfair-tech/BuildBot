...
internal static partial class CloudFormationDeploymentExtractorLoggingExtensions
{
    ...
    [LoggerMessage(eventId: 4, level: LogLevel.Information, message: "CLOUDFORMATION: Logical resource id: {logicalResourceId}")]
    public static partial void LogicalResourceIdFound(this ILogger<CloudFormationDeploymentExtractor> logger, string logicalResourceId);
    ...
    [LoggerMessage(eventId: 7, level: LogLevel.Information, message: "CLOUDFORMATION: Unknown resource status: {status}")]
    ...
    [LoggerMessage(eventId: 11, level: LogLevel.Information, message: "CLOUDFORMATION: Resource Status: {status} : Build Succeeded: {success}")]
    public static partial void ResourceStatus(this ILogger<CloudFormationDeploymentExtractor> logger, string status, bool success);

    [Conditional("DEBUG")]
    [LoggerMessage(eventId: 12, level: LogLevel.Debug, message: "CLOUDFORMATION: Property {key} = \"{value}\"")]
    public static partial void Property(this ILogger<CloudFormationDeploymentExtractor> logger, string key, string value);
}
