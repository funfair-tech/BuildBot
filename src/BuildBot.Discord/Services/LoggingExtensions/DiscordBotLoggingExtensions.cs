...
{
    [LoggerMessage(EventId = 1, Level = LogLevel.Error, Message = "{channelName}: Could not find channel on server {serverName}")]
    public static partial void LogDiscordChannelNotFound(this ILogger<DiscordBot> logger, string channelName, string serverName);

    [LoggerMessage(EventId = 2, Level = LogLevel.Warning, Message = "{channelName}: Sending message {message}")]
    public static partial void LogSendingMessage(this ILogger<DiscordBot> logger, string channelName, string message);

    [LoggerMessage(EventId = 3, Level = LogLevel.Warning, Message = "{channelName}: Sent message {message}")]
